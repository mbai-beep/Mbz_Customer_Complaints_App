...
const COMPLAINT_HEADERS = [
  ...
  'ColorName', 'Contrast', 'Size', 'SoldDate', 'PurReturnDate', 'PurchasedDate',
  ...
  'DepartmentShortName', 'CategoryShortName', 'PurReturnId', 'ItemMRP', 'SalesCost', 'FinalStatus',
  'CustomerName', 'CustomerMobileNumber', 'ReviewedAt', 'ComplaintStatus',
  'SendThrough', 'SendThroughName', 'DocketNumber'
];
...
async function ensureHeader() {
  ...
  // Blank out any stale/duplicate header labels in columns BEYOND our schema, so
  // a leftover column (e.g. an old "Approver"/"Status") can't shadow the real one
  // when rows are read back. Only row 1 beyond our columns is touched; data is safe.
  const colCount = (sheet.properties.gridProperties && sheet.properties.gridProperties.columnCount) || COMPLAINT_HEADERS.length;
  if (colCount > COMPLAINT_HEADERS.length) {
    const from = colLetter(COMPLAINT_HEADERS.length);
    const to = colLetter(colCount - 1);
    await s.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID, range: `${TABS.complaints}!${from}1:${to}1`,
      valueInputOption: 'RAW', requestBody: { values: [Array(colCount - COMPLAINT_HEADERS.length).fill('')] }
    });
  }
}
...
async function getRows(tab) {
  ...
  const rows = values.slice(1).map((arr, i) => {
    const o = { _row: i + 2 };
    header.forEach((h, c) => {
      const val = arr[c] != null ? arr[c] : '';
      // Guard against duplicate / stale header labels: keep the FIRST non-empty
      // value so an empty trailing column can't wipe a real one on read-back.
      if (o[h] === undefined || (o[h] === '' && val !== '')) o[h] = val;
    });
    return o;
  });
  return { header, rows };
...
async function deleteComplaintByTicket(ticketId) {
  ...
  const matches = rows.filter(r => String(field(r, 'TicketID')).trim() === String(ticketId).trim());
