/* Turso (libSQL) access via the built-in HTTP pipeline API (uses global fetch).
   Env: TURSO_DATABASE_URL, TURSO_AUTH_TOKEN.  Used ONLY for the storecode_table login. */
...
async function pipeline(stmts) {
  ...
  for (const r of (j.results || [])) {
    ...
    if (!r.response || r.response.type !== 'execute') continue;   // skip the 'close' response
    const result = r.response.result;
    const cols = (result.cols || []).map(c => c.name);
    const rows = (result.rows || []).map(row => {
      const o = {}; row.forEach((cell, i) => { o[cols[i]] = (!cell || cell.type === 'null' || cell.value === undefined) ? null : cell.value; });
      return o;
    });
    out.push({ cols, rows });
  }
